...
pipeline {
    ...
    stages {
        ...
        stage('Checkout Cloud-Native SG Git Repository') {
            steps {
                git branch: 'master',
	                url: '${GERRIT_MIRROR}/OSS/ENM-Parent/SQ-Gate/com.ericsson.oss.containerisation/eric-enmsg-sevserv'
                sh '''        
                    git remote set-url origin --push https://${GERRIT_HTTP_CREDENTIALS_FUser}@${GERRIT_CENTRAL_HTTP_E2E}/OSS/ENM-Parent/SQ-Gate/com.ericsson.oss.containerisation/eric-enmsg-sevserv
	        '''
            }
        }
        stage('Helm Dep Up ') {
    ...
    post {
        ...
        failure {
            mail to: 'EricssonHyderabad.ENMIPTransportLeptons@tcs.com,EricssonHyderabad.ENMDewdrops@tcs.com',
